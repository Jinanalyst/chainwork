...
/**
 * Lets any signed-in worker set the destination address that ChainWork
 * routes payouts to. Lives in the Payments tab of the worker dashboard
 * and is the only way a LinkedIn-authed worker can receive funds.
 */
export default function PayoutWalletCard() {
  ...
  // Hydrate from the profile row whenever it loads or changes.
  useEffect(() => {
    if (!profile) return
    setChain(profile.payout_chain   || 'base')
    setAddress(profile.payout_address || '')
    setToken(profile.payout_token   || 'USDC')
  }, [profile?.payout_chain, profile?.payout_address, profile?.payout_token])
  ...
  const clear = async () => {
    if (!confirm('Remove your payout wallet? You won\'t receive payouts until you set a new one.')) return
    setBusy(true)
    const res = await update({ payout_address: null, payout_chain: null, payout_token: null })
    setBusy(false)
    if (!res.ok) { alert('Could not remove: ' + (res.error || 'unknown')); return }
    setEditing(false)
  }

  const saved = profile?.payout_address
  const savedChain = chainById(profile?.payout_chain)
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-white/55">
              {saved ? `Active · ${savedChain.label}` : 'No address set'}
            </div>
          </div>
        </div>
        {!editing && saved && (
      ...
      {!editing && saved && (
        <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.18em] text-white/45">{savedChain.label} · {profile.payout_token || savedChain.token}</div>
            <code className="block mt-0.5 font-mono text-sm text-white/85 truncate">{truncate(saved)}</code>
          </div>
      ...
      {(editing || !saved) && (
        <div className="mt-4 space-y-3">
          ...
          <div className="flex items-center gap-2 justify-end">
            ...
            >
              {busy ? 'Saving…' : (saved ? 'Update payout wallet' : 'Save payout wallet')}
            </button>
          </div>
        </div>
      )}
