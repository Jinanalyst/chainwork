  ...
  return (
    <LegalLayout eyebrow="이용약관" title="ChainWork 이용약관" effective={EFFECTIVE}>
      ...
      <LegalSection title="제5조 (서비스의 성격)">
        ...
        결제 대행사(PG)를 통한 원화(KRW) 결제와 작업보호금 보관·정산을 운영하며, 안전한 거래를 위한
        검수 기간, 자동 승인, 분쟁 처리 절차를 제공합니다.</p>
        <p>서비스에서 거래되는 상품은 디자인, 웹/앱 개발, 마케팅 지원, 콘텐츠 제작, 커뮤니티 운영 등
        디지털 전문 서비스에 한정됩니다. 상품권, 게임머니, 포인트, 코인, 토큰 등 현금성 재화는 일체
        거래되지 않습니다.</p>
      </LegalSection>

      <LegalSection title="제6조 (결제 및 정산)">
        <p>결제는 회사가 지정한 PG사를 통해 신용카드, 계좌이체, 간편결제 등으로 이루어집니다. 결제 방식은
        프로젝트 단위, 마일스톤 단위, 월 정기 리테이너 중에서 선택할 수 있습니다. 자세한 사항은
        결제정책에서 정합니다.</p>
